...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}_{}'.format(
        ...
        str(task),
        'orig' if not args.do_aug_train else 'aug'
        ))
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if evaluate:
            ...
            if not args.do_aug_train:
                examples = processor.get_train_examples(args.data_dir)
            else:
                examples = processor.get_augtrain_examples(args.data_dir)
...
def main():
    ...
    parser.add_argument("--separate_evals", action='store_true',
                        help="Set this flag if you are using an uncased model.")
    ...
    if args.do_train and (args.local_rank in [-1,0] or torch.distributed.get_rank() == 0) and not args.tpu:
